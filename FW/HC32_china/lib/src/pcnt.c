...
en_result_t PCNT_Init(stc_pcnt_config_t*  pstcPcntConfig)
{
    ...
    M0P_PCNT->CR_f.DIR = pstcPcntConfig->u8Direc;   //Counting method
...
/**
  * \brief
  * PCNT pulse count setting
  *
  * \param [in] start start counting setting
  * \param [in] end End count setting
  *
  * \retval none
  */
en_result_t PCNT_Parameter(uint8_t start,uint8_t end)
{
  uint32_t u32TimeOut;
   ...
   u32TimeOut = 1000;
   M0P_PCNT->BUF = end;     //Load end overflow value
   ...
   u32TimeOut = 1000;
   M0P_PCNT->BUF = start;     //Load initial value
...
  * \brief
  * PCNT read status
   * \param [in] en_pcnt_status_t PCNT status
  *
  * \retval none
  */
