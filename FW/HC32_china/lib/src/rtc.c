...
/**
  ************************************************** ****************************
  ** \brief RTC count clock selection
  **
  ** \param [in] enClk clock source
  **
  ** \retval Ok
  **
  ************************************************** ****************************/
...
  **
  ** \param [in] pstccCyc cycle interrupt mode and cycle interval selection
  **
  ** \retval Ok
  **
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC time system selection
  **
  ** \param [in] bmode is 12-hour or 24-hour
  **
  ** \retval Ok set up normally
  ** \retval ErrorInvalidParameter setting exception
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC time system acquisition
  **
  ** \param [in] none
  **
  ** \retval time system
...
/**
  ************************************************** ****************************
  ** \brief RTC alarm interrupt settings
  **
  ** \param [in] pstcAlarmTime alarm time, minute, week
  **
  ** \retval Ok set up normally
  **
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC alarm interrupt time acquisition
  **
  ** \param [in] pstcAlarmTime alarm time, minute, week
  **
  ** \retval Ok set up normally
...
/**
  ************************************************** ****************************
  ** \brief RTC 1hz mode selection
  **
  ** \param [in] bmode high precision and normal precision
  **
  ** \retval Ok set up normally
  **
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC 1hz compensation value setting
  **
  ** \param [in] u16Cr compensation value
  **
  ** \retval Ok set up normally
  **
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC function enable setting
  **
  ** \param [in] enFunc function selection
  **
  ** \retval Ok set up normally
  ** \retval ErrorInvalidParameter setting exception
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC function prohibits setting
  **
  ** \param [in] enFunc function selection
  **
  ** \retval Ok set up normally
...
/**
  ************************************************** ****************************
  ** \brief RTC flat, leap year detection
  **
  ** \param [in] u8year year decimal lower two
  **
  ** \retval 1 leap year
  ** \retval 0
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC calculates the number of weeks based on the date
  **
  ** \param [in] pu8Date date
  **
  ** \retval week weeks
  **
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC gets the number of days based on the year and month
  **
...
uint8_t Get_Month_Max_Day(uint8_t u8month, uint8_t u8year)
{
    ...
    return(u8day);//The format of day is bcd code, for example; day is 31 days, day=0x31
}
/**
  ************************************************** ****************************
  ** \brief RTC calculates the number of weeks based on the date
  **
  ** \param [in] pu8buf datetime data, u8len check data length, u8limit_min minimum, u8limit_max maximum
  **
  ** \retval Error error, Ok check is correct
  **
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC time format detection
  **
  ** \param [in] pu8TimeDate datetime data, u8Mode detection mode
  **
  ** \retval enRet check result
  **
  ************************************************** ****************************/
en_result_t Rtc_CheckDateTimeFormat(uint8_t* pu8TimeDate,uint8_t u8Mode)
{
    ...
    while(u8i<7)
    {
        if(u8Mode&&(1<<u8i))
        {
            ...
            {
                case 0:
                    enRet = Check_BCD_Format(*pu8TimeDate,0x00,0x59);//second
                    break;
                case 1:
                    enRet = Check_BCD_Format(*pu8TimeDate,0x00,0x59);//Minute
                    break;
                case 2:
                    if(Rtc12h == M0P_RTC->CR0_f.AMPM)
                    {
                        u8Hour = *pu8TimeDate&0x1f;
                        enRet = Check_BCD_Format(u8Hour,0x00,0x12);//Time
                    }
                    else
                    {
...
  **
  ** \param [in] pstcTimeDate datetime data, bUpdateTime change time, bUpdateDate change date
  **
  ** \retval Ok set up normally
  ** \retval ErrorTimeout timeout error
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
** \brief RTC 12 hours morning or afternoon
  **
  ** \param [in] none
  **
** \retval morning or afternoon
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC get time function
  **
  ** \param [in] pstcTimeDate datetime data
  **
  ** \retval Ok Get normal
  ** \retval ErrorTimeout timeout error
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC count or read and write status acquisition
  **
  ** \param [in] none
  **
  ** \retval count or read and write status
  **
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC Alarm Interrupt Enable
  **
  ** \param [in] enordis interrupt enable or disable
  **
...
en_result_t Rtc_EnAlarmIrq(en_rtc_alarmirq_t enIrqEn)
{
    ...
    Rtc_ClrIrqStatus(RtcAlmf);//Clear interrupt request flag after enabling interrupt
    enRet = Ok;
    return enRet;
}
/**
  ************************************************** ****************************
  ** \brief RTC interrupt request status acquisition
  **
  ** \param [in] Which interrupt request is obtained by enIrqSel
  **
  ** \retval interrupt request status
  **
  ************************************************** ****************************/
...
/**
  ************************************************** ****************************
  ** \brief RTC interrupt request clear
  **
  ** \param [in] enIrqSel clear which interrupt request
  **
  ** \retval Ok Cleared successfully
...
/**
  ************************************************** ****************************
  ** \brief RTC interrupt handler interface acquisition
  **
  ** \param [in] none
  **
  ** \retval interface function address
  **
  ************************************************** ****************************/
static stc_rtc_intern_cb_t* RtcGetInternDataCb(void) 
{
...
/**
  ************************************************** ****************************
  ** \brief RTC overall initialization function
  **
  ** \param [in] pstcRtcConfig initialization structure
  **
...
/**
  ************************************************** ****************************
  ** \brief RTC count disable function
  **
  ** \param [in] none
  **
  ** \retval Ok prohibits setting success
  **
  ************************************************** ****************************/
