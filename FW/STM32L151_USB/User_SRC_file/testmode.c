...
void check_incoming_command(void);
...
void test_loop(void)
{
  ...
  while(1)
  {
    if (cnt_rec_byte != 0) check_incoming_command();
...
{
	uint8_t i;
	for(i = 0; i < cnt_rec_byte; i++) RXbuffer[i] = r_buf[i];
	cnt_rec_byte = 0;
	if ((RXbuffer[0] == HI(RXPREFIX)) && (RXbuffer[1] == LO (RXPREFIX)))
	{
		//check command code
		i = 0;
	while ((COMMANDLIST[i] != 0))
  {
    if (RXbuffer[2] == COMMANDLIST[i])
    {
