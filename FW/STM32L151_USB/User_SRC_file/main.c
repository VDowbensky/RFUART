...
uint8_t cnt_rec_byte = 0;

// буфер в котором хранятся считанные с вирт. com порта данные
uint8_t r_buf[256]; 

// количество байт для передачи
uint8_t cnt_send_byte = 0;

// буфер для передачи на вирт. com порт
uint8_t w_buf[256];
...
char TESTstring[] = "USB-TESTER,0000,0000,X,00000,0000,Y"; 
uint8_t c8[3] = {0,0,0};
int8_t Rssi;
uint8_t Crcok;
...
/*
 void UART_senddata(void)
 {
      ...
      i++;
      TXbuffer[i+13] = 0x0A;
      USART_SendBlock(RxLen+13); //set correct value
      delay_ms(10);
      Green_Off();
 } 
 */
...
void USB_senddata(void)
{
      ...
      i++;
      TXbuffer[i+13] = 0x0A;
      USB_SendBlock(RxLen+15); //set correct value
      delay_ms(10);
      Green_Off();
}	
