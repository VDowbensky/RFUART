...

{
  ...
  case READALL: //checked
    ...
    //RF69_calctemp();
    //if(FreqTest == TRUE) RF69_setopmode (RF_OPMODE_TRANSMITTER);
  ...
  case SETCHANNEL: //checked
    Channel = RXbuffer[3];
		write_eeprom_byte((uint32_t)(&(params->Channel)), (uint32_t)Channel);
    RF69_setchannel(Channel);
    sendOK();
    break;
  ...
  case SETBITRATE: //check!!!
		...
		write_eeprom_word((uint32_t)(&(params->BitRate)), (uint32_t)BitRate);
  ...
  case SETTXPARAMS: //check!!!
    ...
    TxPower = RXbuffer[8];
    //FLASH_Lock(FLASH_MemType_Data);
		write_eeprom_word((uint32_t)(&(params->Deviation)), (uint32_t)Deviation);	
		write_eeprom_byte((uint32_t)(&(params->ModSetting)), (uint32_t)ModSetting);
		write_eeprom_byte((uint32_t)(&(params->TxPower)), (uint32_t)TxPower);
  ...
  case SETRXPARAMS: //check!!!
    ...
    LnaSetting = RXbuffer[11];
		write_eeprom_byte((uint32_t)(&(params->RbwDcc)), (uint32_t)RbwDcc);
		write_eeprom_byte((uint32_t)(&(params->Rbw)), (uint32_t)Rbw);
		write_eeprom_byte((uint32_t)(&(params->AfcDcc)), (uint32_t)AfcDcc);
		write_eeprom_byte((uint32_t)(&(params->Abw)), (uint32_t)Abw);
		write_eeprom_byte((uint32_t)(&(params->AfcSetting)), (uint32_t)AfcSetting);
		write_eeprom_byte((uint32_t)(&(params->LbOffset)), (uint32_t)LbOffset);
		write_eeprom_byte((uint32_t)(&(params->RssiTr)), (uint32_t)RssiTr);
		write_eeprom_byte((uint32_t)(&(params->Dagc)), (uint32_t)Dagc);
		write_eeprom_byte((uint32_t)(&(params->LnaSetting)), (uint32_t)LnaSetting);
  ...
  case SETPACKETCONFIG: //check!!!
    ...
    PkConfig2 = RXbuffer[4];
		write_eeprom_byte((uint32_t)(&(params->PkConfig1)), (uint32_t)PkConfig1);
		write_eeprom_byte((uint32_t)(&(params->PkConfig2)), (uint32_t)PkConfig2);
    sendOK();
    break;
  ...
  case SETADDITIONAL: //check!!!
   ...
   //RxTo = RXbuffer[8];
    ...
    //RxDelay = RXbuffer[12];
		write_eeprom_word((uint32_t)(&(params->PkCount)), (uint32_t)PkCount);
		write_eeprom_byte((uint32_t)(&(params->PkDelay)), (uint32_t)PkDelay);
    RF69_init();
    sendOK();
    break;
  ...
  case SETTEMPOFFSET:
    ...
    TcalValue = RF69_tempmeas();
		write_eeprom_byte((uint32_t)(&(params->Tcal)), (uint32_t)Tcal);
		write_eeprom_byte((uint32_t)(&(params->TcalValue)), (uint32_t)TcalValue);
