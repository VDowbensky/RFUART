...
void USART_init(uint32_t br)
{
		...
		GPIO_PinAFConfig(UARTPORT, TXD_PINSOURCE, GPIO_AF_1); //TXD
		GPIO_PinAFConfig(UARTPORT, RXD_PINSOURCE, GPIO_AF_1); //RXD
...
void USART_SendBlock(uint8_t bl)
{
  ...
  //DE_set(1); //switch driver to transmit
	USART_SendData(USART1, (uint16_t)TXbuffer[0]);
	USART_ITConfig(USART1, USART_IT_TC, ENABLE);
  //TxBusy = 1;
}
...
void USART1_IRQHandler(void)   //USART1 interrupt
{
	...
	//if(USART_GetFlagStatus(USART1, USART_FLAG_TXE) == SET)
		...
		if (TxPointer < TxCount)  	// it's end?
			{
			TxPointer++;			// next byte
			USART_SendData(USART1, (uint16_t)TXbuffer[TxPointer]);
			}
		else 
		{
