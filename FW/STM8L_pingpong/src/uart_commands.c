...

{
  ...
  case SETFREQPARAMS: //check!!!
    ...
    F_Base = *(uint32_t *)(&RXbuffer[3]);
    F_Step = *(uint32_t *)(&RXbuffer[7]);
  ...
  case SETFREQOFFSET: //checked
    ...
    //frequency offset
    FLASH_Unlock(FLASH_MemType_Data);
    F_Offset = *(uint32_t *)(&RXbuffer[3]);
    Tslope = *(int16_t *)(&RXbuffer[7]);
    FLASH_Lock(FLASH_MemType_Data);
    //reload RF69 here
    RF69_setchannel(Channel);
    sendOK();
